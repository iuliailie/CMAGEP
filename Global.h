...
tuple<string,double,vector<double >*> CallCma2(tuple<string,int,vector<double>* > functAndParam, string initialstr,int fTm,vector<double>* constanteOptimizateInainte);
...
enum nodeType { Terminal=0, UnOperator=1,BinOperator=2};

class Node
{
public :
    nodeType type;
    char value;//value when replacing terminals
    char funct;//function
    double param=1;
    Node * left;
    Node * right;
...
static bool isFunction(char a)
{
    if(functionSet.find(a)!=std::string::npos)//we check if the character is in the functions string
        return true;
    else
        return false;
}
...
static double computeTreeValue(Node * node, double *valueArray, map<char,FunctionStructure> *localMap)
{
    if(node->type==Terminal) //if the node  of terminal type
    {
        if(isConstant(node->value))
        {
            ...
            double x= (double)(node->value-'0');
            //cout<<x<<" const"<<endl;
            return x;
        }
        else
            if(IsNumber(valueArray[terminals.find_first_of(node->value)])&&IsFiniteNumber(valueArray[terminals.find_first_of(node->value)]))
            {
                //cout<<valueArray[terminals.find(node->value)]<<"term"<<endl;
                return valueArray[terminals.find_first_of(node->value)];//we return the current value}
            }
    }
    else
        if((node->type==UnOperator)) //if it's an unary function
        {
            ...
            transform(vector1.begin(),vector1.end(),vector1.begin(),vector2.begin(),localMap->at(node->funct).pFunction);
            ...
            if(IsNumber(vector2.at(0))&&IsFiniteNumber(vector2.at(0)))
            {
                ...
                //cout<<localMap->at(node->funct).pFunction<<"\t"<<vector1.at(0)<<" dau \t"<<vector2.at(0)<<endl;
                vector1.clear();
                vector2.clear();
            ...
            //else it's binary
        {
            ...
            vector3.push_back(0);
            transform(vector1.begin(),vector1.end(),vector2.begin(),vector3.begin(),localMap->at(node->funct).pFunction);

            if(IsNumber(vector3.at(0))&&IsFiniteNumber(vector3.at(0)))
            {
                double val=vector3.at(0);
                //cout<<node->funct<<"\t"<<vector1.at(0)<<"si \t"<<vector2.at(0)<<"dau \t"<<vector3.at(0)<<endl;
                vector1.clear();
                vector2.clear();
                vector3.clear();
                //cout<<endl<<"binary"<<val<<endl;
                //cout<<localMap->at(node->funct).pFunction<<"\t"<<vector1.at(0)<<"si \t"<<vector2.at(0)<<"dau \t"<<vector3.at(0)<<endl;
                //cout<<"val="<<val<<endl;
                return val;
            }
        ...
        case '+':
            ...
            if(three)
            {
                if(workNode->left->left->type==Terminal && workNode->left->right->type==Terminal)
                    if(workNode->left->left->value==workNode->right->left->value &&workNode->left->right->value==workNode->right->right->value)
