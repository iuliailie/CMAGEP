...
double QTenHighF(double a, double b)//if this UDF(user defined function) is used, it will take automaticaly as tau value the last element from the terminals vector
{
    double lnQTen=a;
...
PyObject *makeListForParam()
{
    PyObject *l = PyList_New(dataSize);

    for(int i=0; i<dataSize; i++)
    {
        PyObject *interList= PyList_New(nParam);

        for(int j = 0; j< nParam; j++)
        {
            PyList_SetItem(interList, j, PyFloat_FromDouble(*(*actualData+i*nParam+j)));
        }

//      interList= makelist(*(actualData+nParam*i),dataSize);
...
tuple<string,double,vector<double>* > CallCma2(tuple<string,int,vector<double>* > functAndParam, string initialstr,  int fT,vector<double>* constanteOptimizateInainte)
{
...
cout<<endl;
    ...
    PyObject * pModule1 = PyImport_ImportModule("CallCma");
    if(pModule1!=NULL)
    {
...
tuple<string,int,vector<double>*> addParametersInPosition(string strings)
{
    ...
    for(int i=0; i<terminalVector->size(); i++)
    {
        ...
        for(int k=positions.size()-dif; k<positions.size(); k++)
        {
            ...
            if(positions.at(k)<2)
            {
                str.replace(positions.at(k),1,"(H*"+s+")");
                ...
                if(str.substr((positions.at(k)-2),2)!="H*")
                {
                    str.replace(positions.at(k),1,"(H*"+s+")");
    ...
    for(int i=0; i<terminalVector->size(); i++)
    {
        ...
        for(int k=positions.size()-dif; k<positions.size(); k++)
        {
            ...
            if(positions.at(k)<2)
            {
                ...
                if(str.substr((positions.at(k)-2),2)!="P*")
                {
                    str.replace(positions.at(k),1,"(P*"+s+")");
